...
import { getMyClearance } from '@/lib/api/clearance.api';
import { mapClearanceSteps } from '@/lib/api/mappers';
...
function StudentClearancePage() {
  const clearance = useAsync(async () => {
    const { steps } = await getMyClearance();
    return mapClearanceSteps(steps);
  }, []);

  const steps: ClearanceStep[] = clearance.data ?? [];

  const clearedCount = steps.filter((s) => s.cleared).length;
  const total = steps.length;
  const isComplete = total > 0 && clearedCount === total;
  ...
  return (
    <div className="space-y-6">
        ...
        <Card>
          <CardContent className="py-10">
            {clearance.isLoading ? (
              ...
              <ErrorState message={clearance.error} onRetry={clearance.refetch} />
            ) : (
      ...
      {/* Banner */}
      {!clearance.isLoading && !clearance.error && steps.length > 0 && (
                  ...
                  <tbody className="divide-y divide-border">
                    {steps.map((step, i) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5 text-muted-foreground">
                          {step.clearedAt ? new Date(step.clearedAt).toLocaleDateString() : '—'}
                        </td>
                      </motion.tr>
                    ))}
                  </tbody>
