...
import { clearanceRequests as initialRequests } from '@/data/dummy'
...
function BursaryUnitClearancePage() {
  const [requests, setRequests] =
    useState<ClearanceRequest[]>(initialRequests)
  const [searchQuery, setSearchQuery] = useState('')
  const [statusFilter, setStatusFilter] = useState('all')
  const [currentPage, setCurrentPage] = useState(1)
  const ITEMS_PER_PAGE = 5

  const [clearRequest, setClearRequest] =
  ...
  const handleClear = () => {
    ...
    setRequests((prev) =>
      prev.map((r) =>
        r.id === clearRequest.id ? { ...r, cleared: true } : r,
      ),
    )
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <span className="font-medium text-foreground">
                {clearRequest?.studentName}
              </span>
              ? Please confirm the checklist below before proceeding.
            </DialogDescription>
          </DialogHeader>
