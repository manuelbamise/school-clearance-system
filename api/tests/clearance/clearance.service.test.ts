...
import { getMine, clear } from '../../src/clearance/clearance.service';
import {
  buildUser,
  buildClearance,
  buildClearanceUnit,
...
describe('clearance.service', () => {
  ...
  describe('getMine', () => {
    ...
    it('reflects cleared status from clearance units', async () => {
      ...
      prismaMock.clearance.findUnique.mockResolvedValue({
        ...
        units: units.map((u) => ({
          ...u,
          clearedBy: buildUser({ id: 'staff-1', name: 'Staff' }),
        })),
      });
  ...
  describe('clear', () => {
    ...
    it('upserts clearance unit with cleared status', async () => {
      ...
      prismaMock.clearanceUnit.upsert.mockResolvedValue(
        buildClearanceUnit({ status: 'cleared' }),
      );
