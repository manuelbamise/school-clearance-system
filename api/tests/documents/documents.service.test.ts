...
import { review, remove } from '../../src/documents/documents.service';
...
describe('documents.service', () => {
  ...
  describe('review', () => {
    ...
    it('rejects a document with reason', async () => {
      ...
      await review(
        'doc-1',
        { status: 'rejected', rejectionReason: 'Incomplete' },
        'staff-1',
      );
  ...
  describe('remove', () => {
    it('throws 404 if document not found', async () => {
      ...
      await expect(remove('doc-1', 'user-1')).rejects.toThrow(
        'Document not found',
      );
    });
    ...
    it('throws 403 if not recipient', async () => {
      ...
      await expect(remove('doc-1', 'wrong-user')).rejects.toThrow(
        'You can only delete documents sent to you',
      );
    });
    ...
    it('throws if document is still pending', async () => {
      ...
      await expect(remove('doc-1', 'staff-1')).rejects.toThrow(
        'Only reviewed documents can be deleted',
      );
    });
