...
import { authorize, denyRole, authenticateVerified } from '@/middleware/auth.middleware'
...
describe('denyRole middleware', () => {
  it('returns 403 when user role is in the denied list', () => {
    ...
    expect(res.status).toHaveBeenCalledWith(403)
    expect(res.json).toHaveBeenCalledWith({ status: 'error', message: 'Forbidden for this role' })
  })
...
describe('authenticateVerified middleware', () => {
  it('returns 403 with EMAIL_NOT_VERIFIED when user is not verified', () => {
    ...
    if (!u.isVerified) {
      res.status(403).json({ status: 'error', message: 'Email not verified', code: 'EMAIL_NOT_VERIFIED' })
    }
